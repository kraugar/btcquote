...
self._template=["<style>",".btc-last{-webkit-transition: 0.2s linear color; color: #4B4B4B;} .btc-red {color: red;} .btc-green {color: green;} .btc-path {-webkit-transition: 0.2s linear opacty; stroke: 0.2; stroke: #dfdfdf; stroke-width: 2px; fill: none; }",".btc-hides{-webkit-transition: 0.2s linear opacity; opacity: 1;}",".btc-top:hover > .btc-hides {opacity: 0}",".btc-top:hover .btc-path {stroke: #F7931A}",".animated { -webkit-animation-duration: 0.5s; -moz-animation-duration: 0.5s; -o-animation-duration: 0.5s; animation-duration: 0.5s;}","@-webkit-keyframes flash { 0%, 50%, 100% {opacity: 1;} 25%, 75% {opacity: 0;} }","@-moz-keyframes flash { 0%, 50%, 100% {opacity: 1;} 25%, 75% {opacity: 0;} }","@-o-keyframes flash { 0%, 50%, 100% {opacity: 1;} 25%, 75% {opacity: 0;}}","@keyframes flash { 0%, 50%, 100% {opacity: 1;} 25%, 75% {opacity: 0;}}",".animated.flash { -webkit-animation-name: flash; -moz-animation-name: flash; -o-animation-name: flash; animation-name: flash; }","</style>",'<div class="btc-top" style="position: relative; background-color: #DEDEDE; width: 210px; height: 70px; background-image: url('+BACKGROUND_GRADIENT+'); line-height: 1; font-family: Arial; border-radius: 4px; border: 2px solid #D6D4D7;">','<div style="position: absolute; z-index: 2; background-image: url('+BITCOIN_LOGO+'); width: 56px; height: 56px; float: left; margin: 8px 0px 8px 8px;"></div>','<span class="btc-last animated btc-hides" style="position: relative; z-index: 2; font-weight: bold; font-size: 30px; float: right; margin: 8px 8px 0px 0px; height: 30px">${{last}}</span><br />','<span class="btc-hides" style="float: right; padding-right: 8px; height: 14px;">','<span style="color: #999; font-size: 10px; float: left;">Bid: <b>${{bid}}</b></span>','<span style="color: #999; font-size: 10px; float: left; margin-left: 6px;">Ask: <b>${{ask}}</b></span><br />',"</span>",'<div id="btc-sparkline" style="width:220px; height:15px; position: absolute; right: -10px; top: 14px;"></div>','<span style="float: right; font-size: 10px; color: #666; padding-right: 8px;">','Powered by <a href="http://www.btcquote.com" target="_blank" style="color: #666; text-decoration: underline;">BTCQuote.com</a>',"</span>","</div>"].join("\n"),self._dataNames=["last","bid","ask","direction"],self._data={},self._history=[],self.initialize=function(){if(window.d3&&window.Firebase){if(self._widget=document.getElementById("btc-quote"),null===self._widget)throw Exception('Please include a tag with the ID "btc-quote"');self.BTCRef=new Firebase("https://publicdata-bitcoin.firebaseio.com/"),self.BTCRef.child("last").on("value",self.receiveBTCData),self.BTCRef.child("bid").on("value",self.receiveBTCData),self.BTCRef.child("ask").on("value",self.receiveBTCData)}},self.addScript=function(src,callback){var head=document.getElementsByTagName("head")[0],script=document.createElement("script");script.type="text/javascript",script.src=src,script.onload=function(){callback(src)},head.appendChild(script)},self.receiveBTCData=function(snapshot){var name=snapshot.name(),value=parseFloat(snapshot.val()),oldValue=parseFloat(self._data[name]);self.updateData(name,self.formatFloat(value)),self.updateWidget(),"last"==name&&(self.updateHistory(value),self.updateColor(oldValue,parseFloat(value)))},self.updateData=function(name,value){self._data[name]=value},self.updateWidget=function(){var rendered=self._template;for(var nameIndex in self._dataNames){var name=self._dataNames[nameIndex],value=self._data[name]?self._data[name]:"";rendered=rendered.replace(new RegExp("{{"+name.toString()+"}}"),value)}self._widget.innerHTML=rendered,self.displaySparkline()},self.updateHistory=function(value){if(0===self._history.length)for(var i=0;50>i;i++)self._history.push(value);self._history.push(value)},self.formatFloat=function(number){var split=number.toString().split("."),decimal=(void 0!==split[1]?split[1]:"")+new Array(3-(void 0!==split[1]?split[1].length:0)).join("0");return split[0]+"."+decimal},self.updateColor=function(oldPrice,newPrice){var lastElement=self._widget.children[1].children[1];console.log(lastElement,oldPrice,newPrice),oldPrice>newPrice?(lastElement.classList.add("btc-red"),lastElement.classList.add("flash")):newPrice>oldPrice?(lastElement.classList.add("btc-green"),lastElement.classList.add("flash")):self.resetColor(),setTimeout(function(){self.resetColor()},500)},self.resetColor=function(){var lastElement=self._widget.children[1].children[1];lastElement.classList.remove("flash"),lastElement.classList.remove("btc-green"),lastElement.classList.remove("btc-red")},self.displaySparkline=function(){var x,y,line,id="#btc-sparkline",width=220,height=15,interpolation="basis",transitionDelay=1e3,graph=d3.select(id).append("svg:svg").attr("width","100%").attr("height","100%");self._history.length>50&&self._history.shift(),x=d3.scale.linear().domain([0,50]).range([-5,width]),y=d3.scale.linear().domain([d3.max(self._history)+5,d3.min(self._history)-5]).range([0,height]),line=d3.svg.line().x(function(d,i){return x(i)}).y(function(d){return y(d)}).interpolate(interpolation),graph.append("svg:path").attr("d",line(self._history)).attr("class","line"),graph.selectAll("path").data([self._history]).attr("transform","translate("+x(1)+")").attr("d",line).attr("class","btc-path").transition().ease("linear").duration(transitionDelay).attr("transform","translate("+x(0)+")")},self.addScript("http://d3js.org/d3.v2.js",self.initialize),self.addScript("https://cdn.firebase.com/v0/firebase.js",self.initialize)},_bq=new BTCQuote;